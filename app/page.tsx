...
export default function Home() {
  ...
  const topicIdeas = [
    ...
    'How to bake a perfect Pizza at home',
    'Why does the moon look large near the horizon?',
    'What makes Mr. Beast so popular?',
    'What is the difference between Redis and MsSQL'
  ];
  ...
  return (
    <main className="flex min-h-screen flex-col items-left justify-between p-24">
      <div className="w-100">
          ...
          <div className="flex flex-row flex-wrap gap-2">
            {
              topicIdeas.map((idea, index) => {
                return (
                  <button key={index} 
                  className="bg-blue-100 text-blue-800 text-s font-medium me-2 px-2.5 py-0.5 rounded-full dark:bg-blue-900 dark:text-blue-300 border border-blue-400 text-nowrap	"
                  onClick={e => setTopic(idea)}
                  >{idea}</button>
                )
              })
            }
        ...
        <input className="border-2 border-gray-300 p-2 block" value={topic} onInput={e => setTopic((e.target as HTMLTextAreaElement).value)}/>
