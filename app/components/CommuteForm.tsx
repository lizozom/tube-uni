...
import { type TubeStation, type PodcastRecord } from '../types'
...
export function CommuteForm (props: CommuteFormProps) {
  ...
  const [travelTimeMin, setTravelTimeMin] = useState<number | undefined>(
    undefined
  )
  ...
  const [topicPlaceholder, setTopicPlaceholder] = useState<string>(
    props.placeholderTopic
  )
  const [canSubmit, setCanSubmit] = useState<boolean>(false)
  const [submitAction, setSubmitAction] = useState<'generate' | 'load'>(
    'generate'
  )
  ...
  const loadTitle = () => {
    const randomTopic =
      recommendations.current[
        Math.floor(Math.random() * recommendations.current.length)
      ]
      ...
      <div className="flex flex-col flex-wrap content-center">
        ...
        <div className="relative w-full h-fit">
          <textarea
            ...
            placeholder={
              topicPlaceholder
                ? `type something like "${topicPlaceholder}"`
                : ''
            }
            rows={4}
